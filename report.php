...
$curdate=date("Y/m/d");
 ...
 $qry2 = "SELECT count(id) AS total FROM booking WHERE final_stat = '1' AND today = '$curdate' AND stat='1' ";
                        $result = mysqli_query($conn,$qry2) or die("query failed");
                        $values = mysqli_fetch_assoc($result);
                        $num_rows_book = $values['total'];
 $qry3 = "SELECT count(id) AS total FROM booking WHERE stat = '0' AND final_stat = '0' AND today = '$curdate' ";
...
?>
                ...
                <?php
                $conn = mysqli_connect("localhost","root","","hotel") or die("connection failed");
                    $qry = "SELECT count(id) AS total FROM booking WHERE stat = '1' AND final_stat = '0' AND today = '$curdate'   ";
                    ...
                    <div class="panel panel-default">
                                    ...
                                    <div class="panel-heading" >
                                        <h4 class="panel-title">
                                            ...
                                            <button class="btn btn-default"  type="button">
                                                 New Room Bookings today  <span class="badge"><?php echo $num_rows;?></span>
                                            </button>
                                            </a>
                                        </h4>
                                    </div>
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <?php
                    $conn = mysqli_connect("localhost","root","","hotel") or die("connection failed");
                    $qry = "SELECT * FROM booking WHERE stat = '1' AND final_stat = '0' AND today = '$curdate' ";
                    $result = mysqli_query($conn,$qry) or die("query failed");
                                    ...
                                    <div class="panel-heading">
                                        ...
                                        <h4 class="panel-title" style="background-color: #0099cc; border-radius: 5px;">
                                           <form method="post" action="#">
                                                <input class="btn btn-primary" type="submit" value=" Today Booked Rooms" name="searchbtn"> <span class="btn btn-primary btn-sm">
             ...
             $conn = mysqli_connect("localhost","root","","hotel") or die("connection failed");
                      $qry = "SELECT * FROM booking WHERE final_stat = '1' AND today = '$curdate' AND stat='1'  ";
                    $result = mysqli_query($conn,$qry) or die("query failed");
                    if(mysqli_num_rows($result)>0){
                        ?>
                        <div id="mydiv">
                        <table class="table table-bordered table-condensed table-responsive" id="tab2">
                            ...
                            echo "<div class='alert alert-danger'>No Room Booked Today!</div>";
                        }
                        while($row=mysqli_fetch_assoc($result)){
                                ?>
                        <tr>
             ...
             $conn = mysqli_connect("localhost","root","","hotel") or die("connection failed");
                      $qry = "SELECT * FROM booking  WHERE stat = '0' AND final_stat = '0' AND today = '$curdate' ";
                    $result = mysqli_query($conn,$qry) or die("query failed");
                    if(mysqli_num_rows($result)>0){
                        ?>
                        <div id="mydiv2">
